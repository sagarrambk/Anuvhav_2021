    ...
    function(BaseController, Models, Formatter){
        return BaseController.extend("ibm.fin.ar.controller.Main", {
            ...
            onInit: function(){
                ...
                var oRModel = Models.createResourceModel();
                debugger;
                //this.formatter.functionName();
            ...
            onDeleteRow: function(oEvent){
                //Step 1: get all the records of the table from model
                var oModel = sap.ui.getCore().getModel();
                var aData = oModel.getProperty("/empTab");
                //Step 2: get the index of the record which was selected by user
                var sIndex = this.getView().byId("idTable").getSelectedIndices()[0];
                //Step 3: delete the record from the table data 
                aData.splice(sIndex, 1);
                //Steo 4: set the table back to model
                oModel.setProperty("/empTab", aData);
                //Since the SCREEN UI table is bound to the model, the data will be 
                //deleted
            },
            onMagic: function(){
